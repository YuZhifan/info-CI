...
class Login extends CI_Controller {
	...
	public function index() {
		...
		$this->form_validation->set_rules ( 'password', 'Password', 'required|min_length[5]|max_length[15]|callback_password_check', array (
...
// 		$this->load->library ( 'encryption' );
		...
		$this->db->select('id,user_nicename')
				...
				->where(array(
						...
						'user_pass' => hash ( 'md5', $str, false ),
				));
		$query = $this->db->get();
// 		$query = $this->DB_model->select ( "select id,user_nicename from tb_users where user_login = '" . $this->input->post ( username ) . "' and user_pass = '" . hash ( 'md5', $str, false ) . "'" ); // 可用于检测用户名是否已被注册
